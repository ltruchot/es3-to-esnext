...
setTimeout(
  (function(timeToWait) {
    return function() {
      console.log("Elapsed second: " + timeToWait);
    };
  })(wait),
  1000 * wait
);
...
// safe approach
function getPlayerScore(finalScore) {
...
const plrs = [{ name: "Momo54", age: 20 }];
plrs.push({ name: "Chiffon35", age: 22 });
plrs.push({ name: "anonymous1050", age: 25 });
const bibliotheque = plrs.map(function(p) {
  p.team = "pay2win";
  return p;
});
// console.log(bibliotheque);
const total = plrs.reduce(function(accumulateur, p) {
  return accumulateur + p.age;
}, 0);
console.log(total / plrs.length);
const normal = plrs.find(function(p) {
  return p.age === 20;
});
console.log(normal);
const vieux = plrs.filter(function(p) {
  return p.age > 21;
});
console.log(vieux);
